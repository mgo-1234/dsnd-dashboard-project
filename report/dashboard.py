...
from employee_events import QueryBase as QueryBase, Employee as Employee, Team as Team
...
class Header(BaseComponent):
    ...
    def build_component(self, entity_id, model):
        ...
        return H1(getattr(model, "name", "Dashboard"))
...
class LineChart(MatplotlibViz):
    ...
    def visualization(self, asset_id, model):
        ...
        # (assume the first column is the date/day)
...
class BarChart(MatplotlibViz):
    ...
    def visualization(self, asset_id, model):
        ...
        if getattr(model, "name", "").lower() == "team":
            pred = float(proba_pos.mean())
        # Otherwise set `pred` to the first value
        # of the predict_proba output
        else:
            pred = float(proba_pos[0])
